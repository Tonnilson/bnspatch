...
HRESULT Engine_Hooks() {
  if (const auto module = pe::get_module()) {
    ...
    auto result = std::search(data.begin(), data.end(), pattern_searcher("48 8D A8 ?? ?? ?? ?? 48 81 EC ?? ?? ?? ?? 48 C7 45 ?? ?? ?? ?? ?? 48 89 58 ?? 48 89 70 ?? 48 89 78 ?? 0F 29 70 ?? 0F 29 78 ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 85 ?? ?? ?? ?? 4C 8B F9 33 FF 89 7C 24"));
    if (result != data.end()) {
